...
import crypto from '../helpers/compare-password';
...
class authController {
  ...
  static userLogin(req, res) {
    ...
    authService.findUserByUsername(username, 'user').then((user) => {
      ...
      if (user.data[0].active === false) {
        ...
        crypto.compare(password, user.data[0].password).then((response) => {
  ...
  */
  static verifyUser(req, res) {
    ...
    console.log(token);
    authService.verifyUserAccount(token).then((resulter) => {
      return res.status(201).json({
        message: 'Account has been verified',
      });
    }).catch((err) => {
      return res.status(400).json({
        message: 'This Account Could not be Verified',
      });
    });
  }
